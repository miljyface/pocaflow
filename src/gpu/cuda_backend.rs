...
impl CudaContext {
    ...
    pub fn matmul_f32(&mut self, a: &Array2<f32>, b: &Array2<f32>) -> Result<Array2<f32>, i32> {
        ...
        unsafe {
            // Direct copy to persistent pinned buffers (zero intermediate copies)
            ...
            // Optimized cuBLAS call for row-major matrices
            // To compute C = A * B where A is (m x k) and B is (k x n) in row-major:
            // Interpret as column-major: C^T = B^T * A^T
            // So we call: gemm(B^T, A^T) with dimensions swapped
            cublas_error_to_i32(cublasSgemm_v2(
                self.handle,
                cublasOperation_t::CUBLAS_OP_N,  // B^T not transposed (B is already transposed view)
                cublasOperation_t::CUBLAS_OP_N,  // A^T not transposed (A is already transposed view)
                n as i32,        // rows of B^T (cols of B in row-major)
                m as i32,        // cols of A^T (rows of A in row-major)
                k as i32,        // common dimension
                &alpha,
                self.buffer_b as *const f32, n as i32,  // B with leading dim n
                self.buffer_a as *const f32, k as i32,  // A with leading dim k
                &beta,
                self.buffer_c as *mut f32, n as i32,    // C with leading dim n
            ))?;
            ...
            // Zero-copy slice from pinned buffer, then single copy to output
