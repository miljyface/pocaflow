...
// --- Imports; note enums are sometimes inside submodules ---
use cublas_sys::{
    cublasHandle_t, cublasCreate_v2, cublasDestroy_v2, cublasSetStream_v2, cublasSgemm, cublasStatus_t,
};
use cublas_sys::Enum_Unnamed5; // For CUBLAS_OP_N, the 'no transpose' enum

use cuda_sys::cuda::{
    CUcontext, CUdevice, CUstream, CUdeviceptr, CUresult, cuInit, cuDeviceGet, cuCtxCreate_v2,
    cuStreamCreate, cuMemAlloc_v2, cuMemcpyHtoDAsync_v2, cuMemcpyDtoHAsync_v2, cuStreamSynchronize,
    cuStreamDestroy_v2, cuMemFree_v2, cuCtxDestroy_v2
};
use std::ptr;
use ndarray::Array2;
    ...
    pub fn matmul_f32(&self, a: &Array2<f32>, b: &Array2<f32>) -> Array2<f32> {
        ...
        unsafe {
            ...
            cublas_check!(cublasSgemm(
                self.handle,
                Enum_Unnamed5::CUBLAS_OP_N,  // No transpose
                Enum_Unnamed5::CUBLAS_OP_N,  // No transpose
                n as i32,                    // "Rows of B" (output columns)
                m as i32,                    // "Rows of A" (output rows)
